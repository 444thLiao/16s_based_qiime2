...
def join_pairs(manifest,
    ...
    os.makedirs(os.path.join(dir_path, joined_seq_path), exist_ok=True)
    ...
    for i, (sample_id, (fwd_fp, rev_fp)) in enumerate(id_to_fps.iterrows()):
        fastq_out = os.path.join(dir_path, joined_seq_path, sample_id + '.fastq')
...
def _join_pairs_w_command_output(fwd_fp,
    ...
    cmd = [vsearch_path if vsearch_path else '/usr/bin/vsearch',
    ...
    check_call(' '.join(['gzip', fastq_out]), shell=True)
